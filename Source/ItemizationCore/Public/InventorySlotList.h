...
/** Fast array serializer item for a single slot in an inventory. */
...
public:
	...
	UE_API FInventoryItemSlot(uint32 RowIdx, uint32 ColIdx);
	...
	UE_API UItemInstanceBase* GetItemInSlot() const;
	template <class ItemType = UItemInstanceBase>
...
private:
	...
	UPROPERTY()
	TObjectPtr<UItemInstanceBase> ItemInSlot;
...
public:
	...
	UE_API FInventoryItemSlot* FindItemSlot(const UItemInstanceBase* ItemInstance) const;
	...
	int32 Num() const { return ItemSlots.Num(); }
	const auto& operator[](int32 Index) const { return ItemSlots[Index]; }
	TArray<FInventoryItemSlot>::TConstIterator CreateConstIterator() const { return ItemSlots.CreateConstIterator(); }
	TArray<FInventoryItemSlot>::TIterator CreateIterator() { return ItemSlots.CreateIterator(); }
	friend TArray<FInventoryItemSlot>::TConstIterator begin(const FInventorySlotList& Array) { return Array.CreateConstIterator(); }
	friend TArray<FInventoryItemSlot>::TConstIterator end(const FInventorySlotList& Array) { return TArray<FInventoryItemSlot>::TConstIterator(Array.ItemSlots, Array.ItemSlots.Num()); }
	friend TArray<FInventoryItemSlot>::TIterator begin(FInventorySlotList& Array) { return Array.CreateIterator(); }
	friend TArray<FInventoryItemSlot>::TIterator end(FInventorySlotList& Array) { return TArray<FInventoryItemSlot>::TIterator(Array.ItemSlots, Array.ItemSlots.Num()); }
